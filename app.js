...
function localDbRead(){
 ...
 try{
  ...
  const current=raw?localDecode(raw):null;
  if(current)return current;
  const backupRaw=localStorage.getItem(LOCAL_BACKUP_KEY)||"",backup=backupRaw?localDecode(backupRaw):null;
  if(backup){localStorage.setItem(LOCAL_DB_KEY,backupRaw);return backup;}
  if(raw)throw new Error("MarkNest found damaged local data and could not recover it from the backup snapshot. Export or preserve the site data before clearing browser storage.");
 }catch(e){if(e?.message?.startsWith("MarkNest found damaged"))throw e;}
...
function localDbWrite(data){
 ...
 // Keep a valid recovery point. On the first write, the new snapshot itself
 // becomes the initial recovery point; later writes retain the previous state.
 try{localStorage.setItem(LOCAL_BACKUP_KEY,old||serialized)}catch{}
...
function localQuery(table,operation="select",payload=null){
 let filters=[],orderSpec=null,projection=null,single=false,maybeSingle=false;
 const q={
  ...
  then(resolve,reject){Promise.resolve().then(()=>{
   const state=localDbRead();
   if(!Array.isArray(state[table]))return {data:null,error:localStoreError("Unknown local data table.","TABLE_NOT_FOUND")};
   let rows=state[table]||[], data=rows.slice();
   ...
   else if(operation==="update"){const matched=state[table].filter(r=>filters.every(f=>f(r)));if(matched.length){const now=new Date().toISOString();state[table]=state[table].map(r=>filters.every(f=>f(r))?({...r,...payload,updated_at:payload?.updated_at||now}):r);localDbWrite(state);}data=state[table].filter(r=>filters.every(f=>f(r)));}
   else if(operation==="delete"){const removed=state[table].filter(r=>filters.every(f=>f(r)));if(removed.length){state[table]=state[table].filter(r=>!filters.every(f=>f(r)));localDbWrite(state);}data=removed;}
...
async function ensureLabelFamily(title,url,existingFamilyId=null){
 ...
 if(currentDomain&&!familyDomains.some(d=>currentDomain===d||currentDomain.endsWith("."+d))){
  const nextAliases=[...new Set([...(family.aliases||[]).map(normalizeDomain),currentDomain])].filter(d=>d&&d!==normalizeDomain(family.primary_domain)).slice(0,50);
  const previousAliases=(family.aliases||[]).map(normalizeDomain).filter(Boolean);
  const aliasesChanged=nextAliases.length!==previousAliases.length||nextAliases.some((d,i)=>d!==previousAliases[i]);
  if(aliasesChanged){const ur=await store.from("site_families").update({aliases:nextAliases,updated_at:new Date().toISOString()}).eq("id",family.id);if(ur.error)throw ur.error;family.aliases=nextAliases}
 }
 return family.id;
}

function logoDomain(u){
 try{
  const x=new URL(u);
  if(!["http:","https:"].includes(x.protocol))return "";
  const h=x.hostname.toLowerCase();
  if(!h||h==="localhost"||h.endsWith(".local")||h.endsWith(".localhost"))return "";
  if(/^(127\.|10\.|192\.168\.|169\.254\.)/.test(h))return "";
  const parts=h.split(".");
  if(parts.length===4&&parts.every(p=>/^\d+$/.test(p))){
   const n=parts.map(Number);
   if(n[0]===172&&n[1]>=16&&n[1]<=31)return "";
   if(n[0]===100&&n[1]>=64&&n[1]<=127)return "";
  }
  return h;
 }catch{return ""}
}
function logoCandidates(u){
 if(storeGet("marknest-load-logos","off")!=="on")return [];
 try{const x=new URL(u);if(!["http:","https:"].includes(x.protocol)||!logoDomain(u))return [];return [`${x.origin}/favicon.ico`];}catch{return []}
}
function siteLogoUrl(raw){return logoCandidates(raw)[0]||""}
const icon=siteLogoUrl;
function cycleLogoFallback(img){
 const list=(img.dataset.logoCandidates||"").split("|").filter(Boolean);
 const index=Number(img.dataset.logoIndex||"0");
 if(index+1<list.length){img.dataset.logoIndex=String(index+1);img.src=list[index+1];return}
 img.hidden=true;img.closest(".favicon")?.classList.add("faviconFailed");
}
...
function safeHref(u){try{const x=new URL(u);if(!["http:","https:"].includes(x.protocol)||x.username||x.password)return "";return x.href}catch{return ""}}
...
function bookmarkCard(b,selectable=bookmarkSelectionMode){
 const href=safeHref(b.url); const logoList=logoCandidates(b.url); const favicon=safeImageSrc(b.favicon_url);
 ...
 const preferredLogo=logoList[0]||favicon;
 const logoSources=[...new Set([...logoList,...(favicon?[favicon]:[])])];
 const logoAttrs=preferredLogo?`<img src="${esc(preferredLogo)}" alt="" loading="lazy" decoding="async" class="faviconImg" data-logo-candidates="${esc(logoSources.join("|"))}" data-logo-index="0">`:"🔖";
 return `<article class="item ${selected?"selectedItem":""}" data-bookmark-id="${esc(b.id)}" aria-selected="${selected}">${selector}<div class="itemHead"><div class="favicon">${logoAttrs}</div><div class="itemMain"><div class="itemTitle">${esc(b.title)}</div><a class="itemUrl" href="${href?esc(href):"#"}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer">${esc(b.url)}</a><div class="itemTypeBadges"><span class="typeBadge bookmarkTypeBadge">🔖 Bookmark</span>${b.is_favorite?`<span class="typeBadge favoriteTypeBadge">★ Favorite</span>`:""}</div></div></div>
...
function noteCard(n){
 return `<article class="item"><div class="itemTypeBadges"><span class="typeBadge noteTypeBadge">▤ Note</span>${n.is_pinned?`<span class="typeBadge pinnedTypeBadge">📌 Pinned</span>`:""}</div><div class="itemTitle">${n.is_pinned?"📌 ":""}${esc(n.title)}</div><div class="noteContent">${esc(n.content||"")}</div><div class="tags">${cat(n.category_id)?`<span class="tag">${esc(cat(n.category_id))}</span>`:""}${tags(n.tags)}</div><div class="itemActions"><button type="button" class="mini" aria-label="Edit note" data-action="edit-note" data-id="${esc(n.id)}">Edit</button><button type="button" class="mini" aria-label="Delete note" data-action="delete-note" data-id="${esc(n.id)}">Delete</button></div></article>`;
}
function render(){
 fillCatalogCategories();
 ...
 fillCats();fillFamilies();renderSidebarIndex();renderBookmarks();renderNotes();
...
function renderBookmarks(){
 ...
 $("bookmarkGrid").innerHTML=a.length?a.map(b=>bookmarkCard(b,bookmarkSelectionMode)).join(""):`<div class="empty"><strong>No bookmarks found</strong>Try another search or add a bookmark.</div>`;
...
function visibleBookmarks(){
 ...
 return bookmarks.filter(x=>{
  ...
  return categoryMatch&&(!favoritesOnly||x.is_favorite)&&[x.title,x.url,x.description,familyName(x.site_family_id),(x.tags||[]).join(" ")].join(" ").toLowerCase().includes(q);
 });
}
...
async function saveFamily(){
 ...
 await runAction(id?"Updating site family…":"Creating site family…",async()=>{
  ...
  renderFamilies();fillFamilies();resetFamilyEditor();status("Site family saved");closeFamilyManager();
 });
}
...
function captureLocalSnapshot(){return{db:localStorage.getItem(LOCAL_DB_KEY),backup:localStorage.getItem(LOCAL_BACKUP_KEY),meta:localStorage.getItem(LOCAL_META_KEY)}}
function restoreLocalSnapshot(snapshot){const put=(k,v)=>{if(v===null)localStorage.removeItem(k);else localStorage.setItem(k,v)};put(LOCAL_DB_KEY,snapshot.db);put(LOCAL_BACKUP_KEY,snapshot.backup);put(LOCAL_META_KEY,snapshot.meta)}
async function runAction(label,fn){if(actionBusy)return;actionBusy=true;status(label);const snapshot=captureLocalSnapshot();try{await fn()}catch(err){try{restoreLocalSnapshot(snapshot);await load()}catch{}alert(err?.message||"Something went wrong.")}finally{actionBusy=false}}
...
$("form").addEventListener("submit",async e=>{
 e.preventDefault();
 if(saving)return;
 saving=true;let saveCompleted=false;const saveSnapshot=captureLocalSnapshot();$("busy").textContent="Saving…";$("busy").classList.remove("hidden");$("form").classList.add("busy");$("saveError").classList.add("hidden");$("saveButton").disabled=true;
 try{
  ...
  await load();
 }catch(err){try{restoreLocalSnapshot(saveSnapshot);await load()}catch{}$("saveError").textContent=err.message||"Could not save.";$("saveError").classList.remove("hidden")}
...
document.querySelectorAll("[data-view]").forEach(x=>x.onclick=()=>{if(x.dataset.view==="bookmarks")favoritesOnly=false;view(x.dataset.view);renderBookmarks();closeUtilityPanel({restoreFocus:true})});
...
$("bookmarkSearch").oninput=()=>scheduleSearch(renderBookmarks);$("noteSearch").oninput=()=>scheduleSearch(renderNotes);$("bookmarkCategory").onchange=()=>{renderBookmarks();renderSidebarIndex()};$("noteCategory").onchange=renderNotes;
...
document.addEventListener("error",e=>{if(e.target.matches?.(".faviconImg")){cycleLogoFallback(e.target)}},true);
...
function renderSidebarIndex(){
 const total=$("sidebarBookmarkTotal"),notesTotal=$("sidebarNoteTotal"),favoriteTotal=$("sidebarFavoriteTotal"),wrap=$("sidebarCategoryIndex");
 if(total)total.textContent=bookmarks.length;
 if(notesTotal)notesTotal.textContent=notes.length;
 if(favoriteTotal)favoriteTotal.textContent=bookmarks.filter(b=>b.is_favorite).length;
 if(!wrap)return;
 const counts=new Map();
 bookmarks.forEach(b=>{if(b.category_id)counts.set(b.category_id,(counts.get(b.category_id)||0)+1)});
 const rows=categories.slice().sort((a,b)=>a.name.localeCompare(b.name)).map(c=>{
  const count=counts.get(c.id)||0;
  return `<button type="button" class="sidebarIndexLink" data-index-category="${esc(c.id)}"><span>•</span><span>${esc(c.name)}</span><b>${count}</b></button>`;
 });
 wrap.innerHTML=rows.length?rows.join(""):`<div class="sidebarIndexEmpty">No categories yet</div>`;
 const current=$("bookmarkCategory")?.value||"";
 wrap.querySelectorAll("[data-index-category]").forEach(btn=>btn.classList.toggle("active",btn.dataset.indexCategory===current));
 document.querySelectorAll("[data-index-view]").forEach(btn=>{
  const v=btn.dataset.indexView;
  btn.classList.toggle("active",(v==="favorites"&&favoritesOnly)||(v==="bookmarks"&&!favoritesOnly&&$("bookmarksView")&&!$("bookmarksView").classList.contains("hidden"))||(v==="notes"&&!$("notesView").classList.contains("hidden"))||(v==="dashboard"&&!$("dashboardView").classList.contains("hidden")));
 });
}
function initSidebarIndex(){
 const toggle=$("sidebarIndexToggle"),body=$("sidebarIndexBody");
 if(toggle&&body){
  const saved=storeGet("marknest-sidebar-index","open")==="open";
  body.classList.toggle("collapsed",!saved);toggle.setAttribute("aria-expanded",String(saved));toggle.setAttribute("aria-label",saved?"Collapse index":"Expand index");toggle.textContent=saved?"⌄":"›";
  toggle.onclick=()=>{const open=body.classList.contains("collapsed");body.classList.toggle("collapsed",!open);toggle.setAttribute("aria-expanded",String(open));toggle.setAttribute("aria-label",open?"Collapse index":"Expand index");toggle.textContent=open?"⌄":"›";storeSet("marknest-sidebar-index",open?"open":"closed")};
 }
 $("sidebarIndexBody")?.addEventListener("click",e=>{const btn=e.target.closest?.("[data-index-view],[data-index-category]");if(!btn)return;if(btn.dataset.indexView){view(btn.dataset.indexView==="favorites"?"bookmarks":btn.dataset.indexView);if(btn.dataset.indexView==="bookmarks"){$("bookmarkCategory").value="";favoritesOnly=false;}if(btn.dataset.indexView==="favorites"){favoritesOnly=true;renderBookmarks()}renderSidebarIndex();closeUtilityPanel({restoreFocus:true});return}const id=btn.dataset.indexCategory;if(!id)return;view("bookmarks");$("bookmarkCategory").value=id;favoritesOnly=false;renderBookmarks();renderSidebarIndex();closeUtilityPanel({restoreFocus:true})});
}
function setSidebarState(open){
 document.body.classList.toggle("sidebarOpen",open);document.body.classList.toggle("sidebarCollapsed",!open);
 ["sidebarLogoToggle","mobileBrandToggle","topbarBrandToggle"].forEach(id=>{const el=$(id);if(!el)return;el.setAttribute("aria-expanded",String(open));el.setAttribute("aria-label",open?"Hide sidebar":"Show sidebar")});
 document.body.dataset.sidebarOpen=String(open);storeSet("marknest-sidebar",open?"open":"closed");
}
function toggleSidebar(){const isMobile=window.matchMedia("(max-width: 900px)").matches;const open=isMobile?document.body.classList.contains("sidebarOpen"):!document.body.classList.contains("sidebarCollapsed");setSidebarState(!open)}
function initSidebarToggle(){
 const saved=storeGet("marknest-sidebar","closed")==="open";setSidebarState(saved);
 ["sidebarLogoToggle","mobileBrandToggle","topbarBrandToggle"].forEach(id=>$(id)?.addEventListener("click",toggleSidebar));
 window.addEventListener("resize",()=>{const isMobile=window.matchMedia("(max-width: 900px)").matches;const open=isMobile?document.body.classList.contains("sidebarOpen"):!document.body.classList.contains("sidebarCollapsed");setSidebarState(open)});
 const backdrop=$("sidebarBackdrop");backdrop?.addEventListener("click",()=>{if(window.matchMedia("(max-width: 900px)").matches&&document.body.classList.contains("sidebarOpen"))setSidebarState(false)});backdrop?.addEventListener("pointerdown",e=>{if(window.matchMedia("(max-width: 900px)").matches&&document.body.classList.contains("sidebarOpen"))e.preventDefault()});
 document.addEventListener("keydown",e=>{if(e.key==="Escape"&&document.body.classList.contains("sidebarOpen"))setSidebarState(false)});
}
initSidebarToggle();initSidebarIndex();
...
$("bookmarkExportFormat").onchange=e=>storeSet("marknest-export-format",e.target.value);
$("bookmarkExportFormat").value=storeGet("marknest-export-format","json");
$("importBookmarks").onclick=()=>$("bookmarkImportFile").click();
$("bookmarkImportFile").onchange=async e=>{const file=e.target.files?.[0];try{await importBookmarks(file)}catch(err){alert(err?.message||"Could not import bookmarks.")}finally{e.target.value=""}};
document.querySelectorAll('input[name="importCategoryMode"]').forEach(x=>x.addEventListener("change",()=>{const fallbackMode=document.querySelector('input[name="importCategoryMode"]:checked')?.value==="fallback";$("importFallbackWrap").classList.toggle("hidden",!fallbackMode)}));
$("closeImportCategory").onclick=$("cancelImportCategory").onclick=closeImportCategoryDialog;
$("importCategoryModal").addEventListener("click",e=>{if(e.target===$("importCategoryModal"))closeImportCategoryDialog()});
$("confirmImportCategory").onclick=async()=>{if(!pendingImportItems)return;const mode=document.querySelector('input[name="importCategoryMode"]:checked')?.value||"file";const fallback=$("importFallbackCategory").value||null;if(mode==="fallback"&&pendingImportItems.some(x=>!x.category)&&!fallback){alert("Choose an existing category for the uncategorized bookmarks.");return}const items=pendingImportItems;const skip=$("importSkipDuplicates")?.checked!==false;try{await performImport(items,mode,fallback,skip)}catch(err){alert(err?.message||"Could not import bookmarks.")}};

$("manageFamilies").onclick=openFamilyManager;
$("closeFamilyModal").onclick=closeFamilyManager;
$("cancelFamilyEdit").onclick=()=>{resetFamilyEditor();$("familyName").focus()};
$("saveFamily").onclick=async()=>{try{await saveFamily()}catch(e){$("familyError").textContent=e?.message||"Could not save site family.";$("familyError").classList.remove("hidden")}};
$("familyModal").addEventListener("click",e=>{if(e.target===$("familyModal"))closeFamilyManager();const edit=e.target.closest?.("[data-family-edit]");if(edit)editFamily(edit.dataset.familyEdit);const del=e.target.closest?.("[data-family-delete]");if(del)deleteFamily(del.dataset.familyDelete)});
$("siteFamily").onchange=()=>updateFamilySuggestion($("url").value);
document.addEventListener("keydown",e=>{
 if(!$('familyModal').classList.contains('hidden')){
  if(e.key==='Escape'){e.preventDefault();closeFamilyManager();return}
  if(e.key==='Tab'){
   const f=[...$('familyModal').querySelectorAll('button:not([disabled]),input:not([disabled]):not([type=hidden]),textarea:not([disabled]),select:not([disabled]),a[href]')].filter(x=>x.getClientRects().length);
   if(f.length){const first=f[0],last=f[f.length-1];if(!$('familyModal').contains(document.activeElement)){e.preventDefault();first.focus()}else if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();last.focus()}}}
  return
 }
 if($('importCategoryModal').classList.contains('hidden'))return;
 if(e.key==='Escape'){e.preventDefault();closeImportCategoryDialog();return}
 if(e.key==='Tab'){
  const f=[...$('importCategoryModal').querySelectorAll('button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),a[href]')].filter(x=>x.getClientRects().length);
  if(f.length){const first=f[0],last=f[f.length-1];if(!$('importCategoryModal').contains(document.activeElement)){e.preventDefault();first.focus()}else if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus()}}
 }
});
